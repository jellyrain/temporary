...
class Lexical_Analysis:
    ...
    def list_extract(self, str_list: list[str], weight: float = 0.7) -> list[tuple[str, float]]:
        ...
        return [(item[0], (item[1] / 100)) for item in process.extract(self.__lexical, str_list) if (item[1] / 100) >= weight]

    def list_extractOne(self, str_list: list[str], weight: float = 0.7) -> str | None:
        """ 和列表中字符串匹配 返回满足权重且最匹配的值 没有返回None"""
        return process.extractOne(self.__lexical, str_list)[0] if process.extractOne(self.__lexical, str_list)[1] / 100 >= weight else None
